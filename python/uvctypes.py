...
uvc_frame_desc._fields_ = [
    ...
    ("bDescriptorSubtype", c_uint), # enum uvc_vs_desc_subtype bDescriptorSubtype;  # noqa
    # /** Index of the frame within the list of specs available for this format */  # noqa
    ("bFrameIndex", c_uint8),
...
uvc_format_desc._fields_ = [
    ...
    # /** Format specifier */
    ("guidFormat", c_char * 16), # union { uint8_t guidFormat[16]; uint8_t fourccFormat[4]; }  # noqa
...
class uvc_frame(Structure):
    _fields_ = [  # /** Image data for this frame */
        ...
        # /** Number of bytes per horizontal line (undefined for compressed format) */  # noqa
        ("step", c_size_t),
        # /** Frame number (may skip, but is strictly monotonically increasing) */  # noqa
        ("sequence", c_uint32),
        # /** Estimate of system time when the device started capturing the image */  # noqa
        ("capture_time", timeval),
        # /** Handle on the device that produced the image.
        #  * @warning You must not call any uvc_* functions during a callback. */  # noqa
        ("source", POINTER(uvc_device)),
        # /** Is the data buffer owned by the library?
        #  * If 1, the data buffer can be arbitrarily reallocated by frame conversion  # noqa
        #  * functions.
        #  * If 0, the data buffer will not be reallocated or freed by the library.  # noqa
        #  * Set this field to zero if you are supplying the buffer.  # noqa
        #  */
...
def print_device_formats(devh):
    for format_desc in uvc_iter_formats(devh):
        ...
        for frame_desc in uvc_iter_frames_for_format(devh, format_desc):
            print("  frame {0}x{1} @ {2}fps".format(
...
def uvc_get_frame_formats_by_guid(devh, vs_fmt_guid):
    for format_desc in uvc_iter_formats(devh):
        if vs_fmt_guid[0:4] == format_desc.guidFormat[0:4]:
            return [fmt for fmt in uvc_iter_frames_for_format(
                devh, format_desc)]
    return []
